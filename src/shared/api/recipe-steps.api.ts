...
export const recipeStepsApi = {
  ...
  create: async (payload: any) => {
    ...
    const { data } = await http.post<any>('/recipe-steps/', payload);
    return data;
  },

  byRecipe: async (recipeId:number|string) => {

    const { data } = await http.get<any>( `/recipe-steps/?recipe=${recipeId.toString()}`)
    return data;
  },

  getById: async (id: any) => {

    const { data } = await http.get<any>(`/recipe-steps/${id}/`);
    return data;
  },
  ...
  delete: async (id: any) => {
    ...
    await http.delete(`/recipe-steps/${id}/`);
  },

  approve : async (id: any, payload: any) => {

    const { data } = await http.post<any>(
      `/recipe-steps/${id}/approve/`,
      payload
    );
    return data;
  },
  uploadImage : async (id: any, payload: any) => {
    ...
    const { data } = await http.post<any>(
      `/recipe-steps/${id}/recipe-step-imagee/`,
      payload,
      {
    headers: {
