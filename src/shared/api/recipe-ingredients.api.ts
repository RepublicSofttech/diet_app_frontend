...
export const recipeIngredientsApi = {
  get: async (search: any) => {
   ...
   getByRecipe: async (id :any) => {
    const { data } = await http.get<any>(
      `/recipe-ingredients/by-recipe/${id}`
    );
  ...
  create: async (payload: any) => {
    ...
    const { data } = await http.post<any>('/recipe-ingredients/', payload);
    return data;
  },

  getById: async (id: any) => {

    const { data } = await http.get<any>(`/recipe-ingredients/${id}/`);
    return data;
  },
  ...
  delete: async (id: any) => {
    ...
    await http.delete(`/recipe-ingredients/${id}/`);
  },

  approve : async (id: any, payload: any) => {

    const { data } = await http.post<any>(
      `/recipe-ingredients/${id}/approve/`,
      payload
    );
    return data;
  },
  uploadImage : async (id: any, payload: any) => {
    ...
    const { data } = await http.post<any>(
      `/recipe-ingredients/${id}/upload-image/`,
      payload,
      {
    headers: {
